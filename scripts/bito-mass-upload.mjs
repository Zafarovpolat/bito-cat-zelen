...
while ((page-1)*100 < total) {
  ...
  total = d.data.total;
  for (const p of d.data.data) skuMap[String(p.sku)] = p;
  page++;
  await new Promise(r => setTimeout(r, 150));
}
console.log(`Loaded ${Object.keys(skuMap).length} products\n`);
...
for (const file of files) {
  ...
  if (!product) { console.log(`SKIP: ${file}`); skipped++; continue; }
  try {
    ...
    if (up.code !== 0) throw new Error(JSON.stringify(up).slice(0,80));
    const img = up.data;
    ...
    if (upd.code !== 0) throw new Error(JSON.stringify(upd).slice(0,120));
    console.log(`OK  ${product.name} (${sku})`);
    ok++;
    await new Promise(r => setTimeout(r, 250));
  } catch(e) { console.error(`ERR ${file}: ${e.message}`); failed++; }
}
